...
namespace Jitter.Models
{
    public class JitterRepository
    {
        ...
        {
            //SQL: select * from JitterUsers AS users where users.FirstName like %search term% OR useers.LastName like %search term%;  
            var query = from user in _context.JitterUsers select user;
        ...
        public bool CreateJot(JitterUser jitter_user1, string content)
        {
            ...
            try
            {
                Jot added_jot = _context.Jots.Add(a_jot); //Database add row
